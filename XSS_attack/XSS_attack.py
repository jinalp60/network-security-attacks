...
@app.route('/reflected_xss_defense_form')
def reflected_xss_defense_form():
    return render_template('reflected_xss_defense_form.html')
...
@app.route('/reflected_xss_defense', methods = ['POST', 'GET'])
def data_protection():
    if request.method == 'POST':
        form_data = request.form
...
@app.route("/dom_xss_defense")
#http://localhost:5010/dom_xss_defense?value=<script>alert("hi")</script>
def dom_xss_defense():
    to_clean = re.compile('<.*?>')
    value=request.args.get("value")
    ...
    print(cleaned_value)
    r = make_response(render_template('dom_xss_form.html',value=cleaned_value))
    r.headers.set('Content-Security-Policy', "script-src 'none'")
    return r
